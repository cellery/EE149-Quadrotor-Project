...
void p264_encode_int(video_stream_t* const stream, int32_t code)
{
  ...
  if (data != 0)
  {
    ...
    length = 32 - clz5(data);  // number of bits used in level ( = length of level )
    if( length > 1 )
    {
      data   -= 1 << (length - 1);
      length += 1;
...
void p264_encode( video_stream_t* const stream, int32_t level, int32_t run, int32_t not_last )
{
  ...
  if( data > 0 )
  {
    length = 32 - clz5(data);      // compute number of bits used in run ( = length of run )
