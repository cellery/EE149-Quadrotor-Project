...
C_RESULT p264_pack_controller( video_controller_t* controller )
{
  ...
  video_write_data( stream, MAKE_START_CODE_P264(controller->blockline), 22 );
...
C_RESULT p264_unpack_controller( video_controller_t* controller )
{
  ...
  if( start_code == PICTURE_START_CODE_P264 )
  {
...
C_RESULT p264_encode_blockline( video_controller_t* controller, const vp_api_picture_t* blockline, bool_t picture_complete )
{
  ...
  {
    uint32_t add = 32 - clz4(stream->used/controller->blockline);      // estimate the log2 size of a blockline in the stream
    add = 1<<(add+1);                           // major and compute addition buffer size
    stream->bytes = (uint32_t*)vp_os_realloc( stream->bytes, stream->size + add ); // Add some byte to internal stream
    stream->size += add;
  }
  ...
  if (controller->target_size > 0)
  {
    ...
    if (controller->picture_type == VIDEO_PICTURE_INTRA)
      target_size = (NB_P_FRAMES + 1)*controller->target_size/(1+NB_P_FRAMES*(uint32_t)i_p_ratio);
    else
      target_size = (NB_P_FRAMES + 1)*controller->target_size*(uint32_t)i_p_ratio/(1+NB_P_FRAMES*(uint32_t)i_p_ratio);
  ...
  {
    video_stuff8( stream );
    video_write_data( stream, PICTURE_END_CODE_P264, 22 );
  }
