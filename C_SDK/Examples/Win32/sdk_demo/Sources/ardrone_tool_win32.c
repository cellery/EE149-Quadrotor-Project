...
#define CONFIG_ID_BLANK "00000000" 
...
static void ardrone_tool_end_configure( struct _ardrone_control_event_t* event )
{
  ...
  if( configure_data[configure_index].var != NULL && configure_data[configure_index].value != NULL )
  {
	...
	// Win32fix
	ardrone_at_set_toy_configuration_ids( configure_data[configure_index].var,CONFIG_ID_BLANK,CONFIG_ID_BLANK,CONFIG_ID_BLANK,configure_data[configure_index].value );
    ardrone_at_send();
