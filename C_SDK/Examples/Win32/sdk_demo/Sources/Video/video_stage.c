...
/* Global variables to build our video pipeline*/
	...
	vp_os_mutex_t  qr_update_lock;


char string_buffers[8*4296];
int qr_count;
...
C_RESULT output_rendering_device_stage_open( void *cfg, vp_api_io_data_t *in, vp_api_io_data_t *out)
{
	...
	vp_os_mutex_init(&qr_update_lock);
	qr_count = 0;
	return (VP_SUCCESS);
}
...
{
  static uint8_t grey_buff[DRONE_VIDEO_MAX_HEIGHT * DRONE_VIDEO_MAX_WIDTH];
  uint8_t * rgb_src;
  uint8_t * dst;
  int i,j;
  ...
  pixbuf_data      = (uint8_t*)in->buffers[0];
			...
			/** ======= INSERT USER CODE HERE ========== **/
				//send it to the QR Decoder.
				  for(i=0;i<vec->controller.height;i++){
					dst = grey_buff+i*vec->controller.width;
					rgb_src   = pixbuf_data+i*DRONE_VIDEO_MAX_WIDTH*3;
					for (j=0;j<vec->controller.width;j++){
						double r = *(rgb_src++);
						double g = *(rgb_src++);
						double b = *(rgb_src++);
						uint8_t gs = (uint8_t)( .30* r + .59 * g + .11 * b);
						*(dst++)=gs;
					}
				  }
	
				  scan_qr(vec->controller.width,vec->controller.height, grey_buff, string_buffers);
